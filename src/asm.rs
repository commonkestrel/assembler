...
impl OpCode {
    ...
    fn from_instruction(
        ...
        Ok(match instruction {
            ...
            Op::Jnz(addr) => OpCode::triple(
                OP_JNZ,
                Register::A,
                match addr {
                    Address::Literal(addr) => addr,
                    Address::Label(label) => *labels.get(&label.name).ok_or_else(|| {
                        spanned_error!(label.span, "could not find identifier `{}`", label.name)
                    })?,
                },
            ),
            ...
            ILInner::Label(ref label) => {
                    ...
                    match parent.as_ref() {
                        ...
                        None => {
                            errors.push(spanned_error!(
                                il.span.clone(),
                                "sublabel found with no parent label"
                            ));
                            continue;
                        }
                    }
                ...
                if let Some(_) = labels.insert(l, byte) {
                    if label.starts_with('.') {
                        ...
                        errors.push(
                            spanned_error!(il.span.clone(), "duplicate label found")
                                .with_help(format!("consider making this a sublabel: `.{label}`")),
                        );
                    }
                }
            }
